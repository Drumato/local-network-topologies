...
docker exec RR0 vtysh -c 'sh bgp sum json' > bgp-sum/rr0-bgp-sum.json
docker exec RR0 vtysh -c 'sh bgp sum' > bgp-sum/rr0-bgp-sum.out
docker exec RR0 vtysh -c 'sh bgp ipv4 unicast json' > ribs/rr0-ipv4-unicast.json
docker exec RR0 vtysh -c 'sh bgp ipv4 unicast' > ribs/rr0-ipv4-unicast.out
docker exec RR0 vtysh -c 'write mem' > /dev/null
docker exec RR0 bash -c 'cat /etc/frr/frr.conf' > configs/rr0-frr.conf
